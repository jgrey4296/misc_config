...
(defun evil-escape-pre-command-hook ()
  ...
  (message "--- escape pre command: %s %s" real-this-command this-command)
  (with-demoted-errors "evil-escape: Error %S"
      (when (evil-escape-p)
        (let* (;; NOTE Add syl20bnr/evil-escape#91: inhibit redisplay and
          ...
          (message "- escape post let")
          (when inserted
            (evil-escape--delete))
          (message "- escape post delete")
          ...
          (cond
           ...
           ((null evt)
            (message "null event: %s %s %c %s" real-this-command this-command last-command-event unread-post-input-method-events)
            )
