    ...
    (minibuffer-with-setup-hook
      ...
      (setq result
            (read-from-minibuffer
             ...
             t)))
    (dlog! "executing result: %s " result)
...
in case of incomplete or unknown commands."
  (dlog! "-- ex updating: %s %s %s" real-this-command this-command beg)
  (let* ((prompt (minibuffer-prompt-end))
    ...
    (if (and (eq this-command #'self-insert-command)
        ...
        (progn
          ;; (message "Evil ex self insert command")
      ...
      (with-current-buffer evil-ex-current-buffer
        ...
        ;; (message "Post Parser: %s %s" real-this-command this-command)
      ...
      ;; (message "Extracted: %s %s" real-this-command this-command)
      ;; (when (not (eq real-this-command this-command))
      ...
      (dlog! "Cmd, minibufferp: %s %s" cmd (minibufferp))
      (when (and cmd (minibufferp))
        (dlog! "when cmd: %s %s" real-this-command this-command)
...
(defun evil-ex-echo (string &rest args)
  ...
  (dlog! "echo: %s %s" real-this-command this-command)
...
(defun evil-ex-completion-table ()
  (dlog! "Completion table: %s %s" real-this-command this-command)
  (let ((ex-cmds
         (cl-loop
...
(defun evil-ex-completed-binding (command &optional noerror)
  ...
  (dlog! "Completed binding: %s %s" real-this-command this-command)
  (let ((completion (try-completion command evil-ex-commands)))
    (dlog! "completion: %s" completion)
...
Replaces # by the alternate file-name in FILE-NAME."
  (dlog! "replace special filenames")
...
(evil-define-motion jg-evil-goto-line2 (count)
  "Go to line COUNT. By default the last line."
  :jump t
  :type line
  (jg-evil-ensure-column2
  ;; (evil-ensure-column
    (if (null count)
        (goto-char (point-max))
      (goto-char (point-min))
...
(defmacro jg-evil-ensure-column2 (&rest body)
...
(defun jg-evil-line-position (line &optional column)
...
(defun evil-ex-range (beg-line &optional end-line)
  ...
  ;; (message "jg Evil ex range: %s %s" real-this-command this-command)
  (when (and end-line (< end-line beg-line))
    (setq evil-ex-reverse-range t)
    (evil-swap beg-line end-line))
  (let ((range
         (evil-range
          (jg-evil-line-position beg-line)
          (jg-evil-line-position (or end-line beg-line) -1)
          'line
          :expanded t)))
    ;; (message "Got Range: %s %s" real-this-command this-command)
    range
    )
  )
...
START is the start symbol, which defaults to `expression'."
  (dlog! "parse: %s %s" real-this-command this-command)
  (let ((result (funcall
...
(defun evil-ex-command-force-p (command)
  ...
  (dlog! "force: %s %s" real-this-command this-command)
