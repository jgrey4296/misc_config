;; -*- mode:emacs-lisp; -*-
;;; Commentary:
;; adapted from doom's doom-keybinds.el macros
;;
...
(defvar disallow-binds nil "Disallow bindings you don't use. eg: menu's, mouse...")
...
(defvar doom--map-batch-forms nil " Queued alist of (states . bindings) to be finalized")
(defvar doom--map-state '(:dummy t))
(defvar doom--map-parent-state nil)

(defun doom--map-process (rest)
  " return a progn of finalized bindings "
  (let ((doom--map-fn doom--map-fn)
        doom--map-state doom--map-forms desc
        )
    (while rest
      (let ((key (pop rest)))
        (cond ((listp key) ;; handle a list of bindings with no extra details
               (doom--map-nested nil key))

              ((keywordp key) ;; handle keywords
               (pcase key
                 (:leader
                  (doom--map-commit)
              ...
              ((doom--map-def key (pop rest) nil desc) ;; no keywords found, so bind with what state there is
               (setq desc nil)))))
...
(defun doom--map-append-keys (prop)
  " create a keystring from a binding and its parent states  "
...
(defun doom--map-nested (wrapper rest)
  " run the mapping process for potentially wrapped sub bindings "
  (doom--map-commit)
...
(defun doom--map-set (prop &optional value)
  " set the mapping state to a value "
...
(defun doom--map-def (key def &optional states desc)
  " construct the core keybinding (desc?) list, for a set of states, adding to the queued state alist "
...
(defun doom--map-commit ()
  " convert queued bindings to finalized form"
...
(defun doom--map-state ()
  " pair keymaps with their evil-state and prefix "
  (let ((plist
...
(defmacro define-leader-key! (&rest keys)
  ;; instead of using general's define leader key
...
;; TODO add custom state handling
;; TODO move :desc and :state's to after function, to enable sorting
;; TODO only bind if containing module's +bindings flag is set
;; TODO ignore disallowed bindings, +override that
;; TODO record binding file location
;; TODO auto wrap non-interactive commands
;; TODO clear keymap before binding, suppress-keymap
;; TODO provide for sequencing bindings
;; TODO eval-after-load evil tied to a var
;; TODO name the keymap
;; TODO allow mode and state bindings to global leader map
