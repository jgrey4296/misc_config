...
;;         :n :desc "Window Ring Hard Reset" "S" #'window-ring-setup-columns
...
(after! window-ring-minor-mode
  (progn
    (doom--define-leader-key :states 'normal :infix "w r" :desc "Pop Buffer (Alt: Pop to here)" :desc "Clear Ring" :desc "Ring Right" :desc "Ring Left" :desc "Most Recent" :desc "Oldest" :desc "Add Current Buffer" :desc "Find File -> Head" :desc "Find File -> Tail" :desc "Print Sequence" :desc "Window Ring soft Reset" :desc "Window Ring Hard Reset" :desc "Remove Current Buffer" :desc "Toggle Ring Loop")
    (doom--define-leader-key :infix "w r" ""
                             (list :ignore t :which-key "Ring")
                             "c" #'window-ring-pop-buffer "C" #'window-ring-clear-ring "l" #'window-ring-move-perspective "h" #'window-ring-move-perspective-2 "L" #'window-ring-goto-most-recent "H" #'window-ring-goto-oldest "b" #'window-ring-add-current-buffer "f" #'window-ring-add-to-head "F" #'window-ring-add-to-tail "p" #'window-ring-print-order "s" #'+window-ring-block-reset "S" #'window-ring-setup-columns "r" #'window-ring-remove-buffer "q"
                             #'(lambda nil
                                 (interactive)
