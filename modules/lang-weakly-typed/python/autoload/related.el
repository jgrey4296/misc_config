...
(make-related! jg-python-project
  :files ((:impl (f-join root fparent2 (s-replace "test_" "" fname)) :when is-test)
          ...
          (:api (f-join fparent "_interface.py") :when (f-exists? (f-join root fparent "_interface.py")))
          (:api (f-join fparent2 "_interface.py") :when (f-exists? (f-join root fparent2 "_interface.py")))
          (:doot "doot.toml")
