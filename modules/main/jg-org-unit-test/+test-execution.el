...
(defun jg-org-test-run-test-group (testgroup)
  (assert (jg-org-test-test-group-p testgroup) t)
  (goto-char (jg-org-test-test-group-start testgroup))
  (let* ((name (jg-org-test-test-group-name testgroup))
         (bound (jg-org-test-test-group-bound testgroup))
         (results (mapcar (lambda (x) `(,(jg-org-test-test-to-string x) . ,(jg-org-test-run-test x bound))) (jg-org-test-test-group-tests testgroup)))
         )
    (make-jg-org-test-test-results :name name :results results :link (jg-org-test-test-group-link testgroup)))
  )
(defun jg-org-test-run-test (test bound)
  (assert (jg-org-test-test-p test))
  (save-excursion
    (condition-case e
        (let ((type (jg-org-test-test-type test)))
          (cond
...
(defun jg-org-test-run-test-section (test bound)
  ;;go to the locator
  (jg-org-test-forward-past-tests (jg-org-test-test-locator test) bound)
  ;; get subheadings
  (let ((subheadings (org-map-entries
    ...
    (-contains? subheadings (jg-org-test-test-value test))
    )
  )
(defun jg-org-test-run-test-length (test bound)
  (jg-org-test-forward-past-tests (jg-org-test-test-locator test) bound)
  (let* ((vals (jg-org-test-test-value test))
         (dir (car vals))
         (length (cadr vals))
         (counter (caddr vals))
...
(defun jg-org-test-run-test-order (test bound)
  (let* ((tree (org-map-entries (lambda () (substring-no-properties (org-get-heading))) nil 'tree))
         (locator (-elem-index (jg-org-test-test-locator test) tree))
         (section (-elem-index (jg-org-test-test-value test) tree))
         )
...
(defun jg-org-test-run-test-citation (test bound)
  (jg-org-test-forward-past-tests (jg-org-test-test-locator test) bound)
  (let ((citations (jg-org-test-test-value test)) curr-line)
    (while citations
...
(defun jg-org-test-run-test-mention (test bound)
  (jg-org-test-forward-past-tests (jg-org-test-test-locator test) bound)
  (search-forward (jg-org-test-test-value test) bound)
  )
(defun jg-org-test-run-test-codeblock (test bound)
  (jg-org-test-forward-past-tests (jg-org-test-test-locator test) bound)
  (re-search-forward (format "%s %s" jg-org-test-src-block-regexp
                             (if (jg-org-test-test-value test) (jg-org-test-test-value test) "")) bound)
  )
(defun jg-org-test-run-test-tag (test bound)
  (jg-org-test-forward-to (jg-org-test-test-locator test) bound)
  (let ((tags (plist-get (cadr (org-element-at-point)) :tags)))
    (-contains? tags (jg-org-test-test-value test))
    )
  )
