...
(defun jg-tag-unify-layer-candidates ()
  ...
  (let* ((buffer-cand-tags (jg-tag-get-buffer-tags))
    ...
    (if (not (hash-table-empty-p global-tags))
        (let* ((cand-keys (hash-table-keys global-tags))
          ...
          (setq jg-tag-unify-layer-candidate-counts global-tags)
          (setq jg-tag-unify-layer-candidates-names (sort propertied-tags 'jg-tag-sort-candidates))
          )
      '()
      ))
  )
