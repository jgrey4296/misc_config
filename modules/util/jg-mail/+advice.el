...
(define-advice rmail-header-summary (:override ()
                                     +jg-mail-header-summary)
...
(define-advice rmail-create-summary (:override (msgnum deleted unseen lines) +jg-mail-create-summary)
  (goto-char (point-min))
...
(define-advice rmail-new-summary-1 (:around (fn &rest rst)
                                    +jg-mail-new-summary-princ-override)
...
(define-advice rmail-summary-update-line (:around (fn &rest rst)
                                          +jg-mail-summary-update-princ-override)
  (if (function-get 'princ 'original)
      (unwind-protect
          (progn (fset 'princ (function-get 'princ 'mod))
                 (apply fn rst))
