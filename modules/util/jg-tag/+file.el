...
(defun jg-tag-chop-long-file (name &optional preferred-length)
  "Take long org files and split them into multiple files
If preferred-length is not specified, use jg-tag-preferred-linecount-for-org
"
  (message "----------")
    ...
    (let* ((count 1)
      ...
      (while (re-search-forward "^\\*\\*[^*]" nil t )
        ...
        (setq linecount (+ linecount (funcall ln-fn (point) last-position))
              last-position (point))
        (if (> linecount (or preferred-length jg-tag-preferred-linecount-for-org))
            (setq linecount 0
                  count (+ 1 count))
          )
        )
...
(defun jg-tag-chop-long-files-from-dired ()
  "Subdivide marked files if they are too long"
  (interactive)
  (let ((files (dired-get-marked-files)))
    (seq-each 'jg-tag-chop-long-file files)
    )
  )
